...
class Register : AppCompatActivity() {
    private var mutableList = mutableListOf<Entities>()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_register)
            mutableList = intent.getSerializableExtra("users") as MutableList<Entities>
    }
    fun onContinue(v: View){
        ...
        else if(confirmPassword.text.isNullOrBlank() || confirmPassword.text.toString().count() < 6)
        {
            ...
            if(mutableList.any{entities -> entities.email == email.text.toString()})
            {
                Toast.makeText(this, "Email already exists", Toast.LENGTH_LONG).show()
                return
            }
            if(mutableList.any{entities -> entities.name == username.text.toString()})
            {
            ...
            mutableList.add(Entities(username.text.toString(), email.text.toString(), password.text.toString() ))
            val intent = intent
